    ...
    <div className={`min-h-screen bg-gradient-to-b ${result.dungeon.theme.bg} flex flex-col items-center justify-center p-6 gap-6 relative overflow-hidden`}>
      ...
      <div className="text-center animate-popIn relative z-20">
        <div className="flex justify-center gap-2 text-6xl">
          {[1,2,3].map(i => (
            <span key={i} className={`${i <= result.stars ? 'text-yellow-400 drop-shadow-[0_0_10px_rgba(250,204,21,0.8)]' : 'text-gray-400/30'}`}>{'\u2605'}</span>
          ))}
        </div>
        <div className="text-gray-900 text-xl font-bold mt-3">{message}</div>
      </div>

      <div className="bg-white/40 backdrop-blur rounded-3xl p-6 border border-white/50 w-full max-w-sm relative z-20">
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <div className="text-green-600 text-3xl font-black">{result.correctCount}</div>
            <div className="text-gray-600 text-sm">Correct</div>
          </div>
          <div>
            <div className="text-red-600 text-3xl font-black">{result.wrongCount}</div>
            <div className="text-gray-600 text-sm">Wrong</div>
          </div>
          <div>
            <div className="text-yellow-600 text-3xl font-black">{accuracy}%</div>
            <div className="text-gray-600 text-sm">Accuracy</div>
          </div>
        </div>
        <div className="border-t border-gray-300 mt-4 pt-4 text-center">
          <div className="text-yellow-600 font-bold">+{result.xpEarned} XP earned!</div>
